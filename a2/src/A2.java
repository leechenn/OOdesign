...
/* NetIds: cl953. Time spent: 2 hours. */
...
public class A2 {
    ...
    public static boolean isMidSame(String s) {
    	...
    	int length = s.length();
    	if(length%2 == 0 && s.charAt(length/2)==s.charAt(length/2-1)) {
    		return true;
    	}
    	...
    	if(s.length()>0) {
    	   ...
    	   for(int i = 0;i<length;i++) {
    		   ...
    		   if(97<=(int)s.charAt(i)&&(int)s.charAt(i)<=122) {
    			   char ch= s.charAt(i);
    			   init += String.valueOf(ch).toUpperCase();
    		   }
    	   }
    	   return init;
    	}
    ...
    public static boolean atLeastTwo(String s, String s1) {
    	...
    	if(s.length()>0&&s1.length()==0) {
    		...
    		if(s.length()==0&&s1.length()==0) {
    			...
    			if(s.contains(s1)) {
    				if(s.substring(s.indexOf(s1)+s1.length()).contains(s1)) {
    					return true;
    				}
    			}
    		}
    ...
    public static boolean isX(String s, int n) {
    	...
    	if(s.length()==0||s.length()<n||s.length()%n!=0) {
    		return false;
    	}
    	boolean flag = true;
    	for(int i=0;i<s.length();i=i+n) {
    		String subString = s.substring(0, n);
    		if(!s.substring(i,i+n).equals(subString)) {
    			flag = false;
    		}
